...
final class Teacher extends Model
{
    ...
    {
        $intranet = Database::getIntranetConnection();
        $rows = $intranet->query(
            "SELECT p.id, CONCAT(p.nombre,' ', p.apellidos) AS nombre_completo,
                    p.telefono, p.email, p.foto, pr.clase_tutoria_codigo
            FROM personal p
            INNER JOIN profesores pr ON p.id = pr.personal_id"
        )->fetchAll(PDO::FETCH_ASSOC);

        $sql = "INSERT IGNORE INTO teachers
                (id, full_name, email, phone, profile_img_path, is_tutor, enabled)
                VALUES (?, ?, ?, ?, ?, ?, 1)";
        ...
        foreach ($rows as $row) {
            $stmt->execute([
                ...
                $row['telefono'],
                $row['foto'],
                $row['clase_tutoria_codigo'] !== null ? 1 : 0,
            ]);
        }
    }
}
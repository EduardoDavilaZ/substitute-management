<?php
function _header_jwt_payload_evg(string $token): ?array {
    $parts = explode('.', $token);
...
$_h_payload = !empty($_COOKIE['auth_token']) ? _header_jwt_payload_evg($_COOKIE['auth_token']) : null;
$_h_d       = $_h_payload['data'] ?? null;
$_h_nombre  = $_h_d['nombre'] ?? null;
$_h_foto    = $_h_d['foto']   ?? null;

$_h_logo      = $headerLogo      ?? ASSETS_URL . '/img/logo.png';
$_h_appNombre = $headerAppNombre ?? 'Gestión de Guardias';
$_h_appSub    = $headerAppSub    ?? 'Escuela Virgen de Guadalupe';
$_h_home      = $headerHome      ?? BASE_URL;
$_h_logout    = $headerLogout    ?? BASE_URL . 'auth/logout';
?>
<style>
.evg-header {
